...
2. Call read_page to see current state
3. If multiple datasets: click the dataset selector dropdown with click_element
...
```text
1. Call read_page to see available episodes in the sidebar
2. Click the episode button with click_element
3. Call read_page to confirm main content updated
4. Take a screenshot with screenshot_page to verify
```

### Switch Dataset

```text
1. Click the dataset selector in the header with click_element
2. Select the target dataset option
3. Call read_page to confirm episode list reloaded
...
```text
1. Call read_page to inspect the current UI state
2. Check terminal output for backend errors
3. Take a screenshot with screenshot_page if needed
```
...
Use `navigate_page` or `curl` for direct API inspection:
