...
class TestDetectionService:
    ...
    def test_detect_synthetic_image(self):
        ...
        import asyncio
        result = asyncio.run(
            service.detect_frame(image_bytes, frame_idx=0, confidence=0.25)
        )
    ...
    def test_detect_person_image(self):
        ...
        test_images_path = os.path.join(
            os.path.dirname(model.model_name or ""),
            "assets"
        )
        ...
        import asyncio
        result = asyncio.run(
            service.detect_frame(image_bytes, frame_idx=0, confidence=0.1)
        )
